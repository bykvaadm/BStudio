...
| `studio-launch.command`  | двойной клик — поднимает локальный сервер и открывает студию    |
| `example/`               | готовый учебный урок на 4 слайда — открывай студию и пробуй сразу |

## Что нужно студии

Ровно две вещи, лежащие рядом в папке урока:

1. **Картинки слайдов** — обычные PNG в подпапке `deck_frames/`, по имени `p-01.png`, `p-02.png`, … Чем ты их сделал,
   студии всё равно — Keynote, Figma, PowerPoint, экспорт из чего угодно, лишь бы на выходе были файлы.
2. **Текстовый файл с речитативом** — `03-script.md`, где написано, что говорить на каждом слайде.
...
Студия также умеет открыть **PDF** рядом с уроком (рендерит страницы на лету через `pdf.js` с CDN — нужен интернет) и
...
- Python 3 — `studio-launch.command` поднимает им простой локальный сервер (камера в браузере работает только через
  `http://localhost`, не через `file://`).
...
Урок «виден» студии, если в нём есть **`03-script.md`** и где-то внутри **`deck_frames/`** с кадрами (или PDF рядом).

## Флоу

Двойной клик по **`studio-launch.command`** → откроется Chrome:
...
- **Запись:** ⏺ → отсчёт **3-2-1** → пишет. ⏸ пауза/продолжить → ⏹ стоп. Видео **пишется сразу на диск** в папку урока (
  ...
  микрофон (дропдаун выбора микрофона). Суфлёр в видео **не попадает** (надо — «вшить текст»). Разрешение 1080p/720p.
  Закрыть вкладку во время записи браузер не даст молча. **⚑ отметка** — метка-таймкод «тут косяк / переснимал»; паузы и
