...
- **«сброс камеры»** — возвращает базу к заводской: низ-право, стандартный размер, прямоугольник, без зума. База — это
  то, что видно на слайдах без `meta:camera`; слайд с метой переопределяет её только для себя. Если у текущего слайда
  мета есть, после сброса сверху на ~6 c покажется баннер с тем, что она рекомендует.
...
- **Тест синхрона (`studio.html?synctest`):** диагностика расхождения звука и видео. Во время записи каждые 10 с
  квадрат в левом верхнем углу меняет цвет, и в тот же момент в колонки уходит короткий щелчок — микрофон слышит его
  через воздух. Рядом с дублем ложится `*.synctest.json` (настройки микрофона, задержки, формат). Писать с колонками,
  не в наушниках, молча и без пауз. Квадрат рисуется прямо в кадр и камеру не проверяет — чтобы поймать задержку
  самой камеры, хлопни в ладоши в кадре в начале, в середине и в конце.
