...
2. **Текстовый файл с речитативом** — `03-script.md`, где написано, что говорить на каждом слайде. **Необязателен:**
   без него урок всё равно откроется, в списке будет помечен «без речитатива», суфлёр останется пустым, а слайды,
   камера, отметки и запись работают как обычно.
...
Речитатив рядом — по желанию. Поэтому студии можно скормить **любую папку с картинками**: она откроется как урок без
